...
### Changed
- **Project Structure**: Engine is now a static library, Game project is the executable
- **Build Process**: Use the Game project to build instead of Engine

## [0.1.5] - 29-03-2026

### Added
- **Scene Serialization**: New `CScene::Save()` and `Load()` methods for scene persistence
- **Scene Loading**: Added scene loading from JSON files in Game/Assets/Scenes/
- **Toolbar Window**: New `EToolbarWindow` with editor mode controls (Play, Pause, Stop)
- **Entity Play Mode Updates**: Entities now properly update when switching to play mode
- **JSON Enhancements**: Expanded JSON utilities for scene serialization

### Fixed
- **Scene Load Crash**: Fixed race condition crash when loading scenes
